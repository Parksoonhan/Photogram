		...
		<div class="profile-right">
			<div class="name-group">
				...
				<c:choose>
					<c:when test="${dto.pageOwnerState}">
						...
						<button class="cta" onclick="location.href='/user/getmate'">일정등록</button>
					</c:when>
					<c:otherwise>
						<c:choose>
			...
			<div class="subscribe">
				<ul>
					...
					<li><a href="/user/maplist/${dto.user.id}"> 일정<span>${dto.mapCount}</span>
					</a></li>
			...
			<c:forEach var="image" items="${dto.user.images}"> <!--EL 표현식에서 변수명을 적으면 get함수가 자동 호출된다. -->
				<div class="img-box">
					...
					<div class="comment">
						...
						<c:if test="${dto.pageOwnerState}">
							<a href="#" onclick="deleteImage(${image.id})" style="padding-left: 80%; position: absolute; padding-bottom: 80%;">x</a>
						</c:if>
					</div>
				</div>
			</c:forEach>
